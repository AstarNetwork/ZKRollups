...
if [ $1 = "actions" ]; then
    ...
    DOCKER_BUILDKIT=1 docker build --ssh default=$HOME/.ssh/id_rsa -f ts-tests/tests/zksync/docker/server/Dockerfile zksync -t docker.pkg.github.com/plasmnetwork/zkrollups/operator:latest
    ...
    DOCKER_BUILDKIT=1 docker build --ssh default=$HOME/.ssh/id_rsa -f ts-tests/tests/zksync/docker/prover/Dockerfile zksync -t docker.pkg.github.com/plasmnetwork/zkrollups/prover:latest
    ...
    DOCKER_BUILDKIT=1 docker build --ssh default=$HOME/.ssh/id_rsa -f ts-tests/tests/zksync/docker/server/Dockerfile zksync -t zksync/server
    DOCKER_BUILDKIT=1 docker build --ssh default=$HOME/.ssh/id_rsa -f ts-tests/tests/zksync/docker/prover/Dockerfile zksync -t zksync/prover
fi
